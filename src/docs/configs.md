...
| `configs.base.typescript`   | `.ts`, `.tsx`, `.cts`, `.mts`, `.d.ts` | TypeScript parser and type-aware rules    |
| `configs.react.typescript`  | `.ts`, `.tsx`, `.cts`, `.mts`, `.d.ts` | React adjustments for TypeScript          |
...
`react/jsx-filename-extension`, and a repeat of the TypeScript import settings
that `configs.base.typescript` already sets.
...
Every composition parses the latest ECMAScript syntax. `airbnb-extended` pins
espree to ES2018, which `configs.base.recommended` overrides; without it a
JavaScript project could not use optional chaining or class fields.
