# Migration from a legacy setup

Moving a project from `.eslintrc*` with `eslint-config-airbnb` to this config.
The config is flat config only and extends `eslint-config-airbnb-extended`;
